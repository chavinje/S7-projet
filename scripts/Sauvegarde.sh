...
sudo echo "0 1 * * *  mysqldump -h ${BDD_HOST} -u ${BDD_USER} -p${BDD_PASS} ${BDD_NAME} > /vagrant/files/backup_$(date +\%Y\%m\%d).sql" | sudo sed 's/$(date +\%Y\%m\%d)/`date +\%Y\%m\%d`/' > /tmp/cron_backup.sh
sudo echo "0 0 * * 1  find /vagrant/files/backup_* -mtime +7 -delete" >> /tmp/cron_backup.sh
